...
const CLASS_MAP: Record<string, string> = {
  ...
  card: 'k-card', panel: 'k-panel', button: 'k-button', input: 'k-input', textarea: 'k-textarea', select: 'k-select', badge: 'k-badge', image: 'k-image', avatar: 'k-avatar', table: 'k-table'
};
...
function renderDocument(input: {
  ...
  themeTokens: PropertyNode[];
  metaTokens: PropertyNode[];
}): string {
...
.k-badge { display: inline-flex; width: fit-content; padding: 6px 10px; border-radius: 999px; background: #eef2ff; color: #3730a3; font-weight: 800; }
...
@media (max-width: 720px) { .k-screen { padding: 18px; } .k-section { padding: 36px 16px; } }`;
}
