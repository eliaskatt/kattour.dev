...
export function compile(source: string): string {
  ...
  for (const node of ast.body) if (node.type === 'Component') components.set(node.name, node);

  const previewState = { ...state };
  for (const [name, expression] of Object.entries(computedMap)) previewState[name] = interpolateComputed(String(expression), previewState);
...
function renderFor(node: ForNode, ctx: CompileContext, scope: RuntimeScope): string {
  ...
  return items.map(item => node.body.map(child => renderNode(child, ctx, { ...scope, [node.item]: item })).join('')).join('');
}
function renderElement(element: ElementNode, ctx: CompileContext, scope: RuntimeScope): string {
  ...
  if (component) {
    ...
    component.params.forEach((param, index) => { if (index === 0 && element.label) nextScope[param] = evaluateExpression(element.label, ctx.state, scope); });
  ...
  for (const event of element.events) if (event.name === 'click') attrs.push(`data-k-click="${escapeHtml(event.action)}"`);
